...
#print(full_path.rsplit('/', 1)[0])
try:
    ctypes.cdll.LoadLibrary(full_path.rsplit('/', 1)[0] + '/plugins/libyolo_layer.so')
except OSError as e:
...
class TrtYOLO(object):
    ...
    def _load_engine(self):
        TRTbin = '%s/%s.trt' % (self.path, self.model)
        with open(TRTbin, 'rb') as f, trt.Runtime(self.trt_logger) as runtime:
            return runtime.deserialize_cuda_engine(f.read())

    def __init__(self, path, model, category_num=80, letter_box=False, cuda_ctx=None):
